...
// Inline glossary term: dotted underline + a hover tooltip with the definition.
export function Term({ k, children, style }: { k: string; children?: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <span title={G[k] ?? k} style={{ borderBottom: `1px dotted ${C.faint}`, cursor: 'help', ...style }}>
      {children ?? k}
    </span>
  );
}
...
// Section heading inside the control plane. `tip` adds a hover definition.
export function Section({ title, hint, tip, children }: { title: string; hint?: string; tip?: string; children: React.ReactNode }) {
  return (
    <div style={{ marginTop: 22 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 10 }}>
        <span title={tip} style={{ fontFamily: MONO, fontSize: 12, letterSpacing: '0.16em', textTransform: 'uppercase', color: C.green, cursor: tip ? 'help' : 'default', borderBottom: tip ? `1px dotted rgba(118,185,0,0.4)` : 'none' }}>{title}</span>
