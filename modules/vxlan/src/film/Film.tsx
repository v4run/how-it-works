// "VXLAN Explained" — the diagrammatic explainer film. Ported faithfully from
// the Claude Design prototype (vxlan-video.jsx) onto the project's own timeline
// engine (Stage / Sprite / useSprite). Scenes read sprite-local time.
//
// Colour language: green = overlay / VXLAN, steel-blue = underlay / physical
// fabric, amber = flood / legacy / warning.
import React from 'react';
import { C as T, MONO, DISP, UI } from '../design/theme';
import { Easing, clamp, interpolate } from '../engine/anim';
import { Stage, Sprite, useSprite, useTime } from '../engine/timeline';

// Film palette (a superset alias over the shared tokens, plus a few film-only).
const C = {
  bg: T.bg,
  panel: T.panel,
  panel2: T.panel2,
  ...
  greenGlow: T.greenGlow,
  text: T.ink,
  dim: T.dim,
  faint: T.faint,
  steel: T.steel,
  steelSoft: T.steelSoft,
  steelLine: T.steelLine,
  amber: T.amber,
  amberSoft: T.amberSoft,
  amberLine: T.amberLine,
  red: T.red,
};
const F = { d: DISP, s: UI, m: MONO };
...
// eased 0..1 gate
function gate(localTime: number, at: number, dur = 0.5, ease: (t: number) => number = Easing.easeOutCubic) {
  return ease(clamp((localTime - at) / dur, 0, 1));
}

// ── scene shell: padding + fade-in/out envelope ──────────────────────────────
function SceneFrame({ children, pad = '120px 132px 96px' }: { children: React.ReactNode; pad?: string }) {
  const { localTime, duration } = useSprite();
  const fin = clamp(localTime / 0.5, 0, 1);
  const fout = clamp((duration - localTime) / 0.5, 0, 1);
  const op = Math.min(fin, fout);
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: op, padding: pad, boxSizing: 'border-box' }}>{children}</div>
  );
}

function SceneTitle({ title, sub, at = 0, maxW = 1320 }: { title: React.ReactNode; sub?: React.ReactNode; at?: number; maxW?: number }) {
  return (
...
// chip / pill label
function Chip({ children, color = C.green, solid = false, style }: { children: React.ReactNode; color?: string; solid?: boolean; style?: React.CSSProperties }) {
  return (
    <span
      style={{
        font: `500 13px ${F.m}`,
    ...
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        {word.split('').map((ch, i) => {
          ...
          return (
            <span key={i} style={{ font: `700 168px ${F.d}`, color: C.text, letterSpacing: '-0.03em', opacity: clamp(p, 0, 1), transform: `translateY(${(1 - p) * 40}px)`, display: 'inline-block' }}>
              {ch}
            </span>
          );
        })}
...
function SceneProblem() {
  ...
  return (
    <SceneFrame>
      <SceneTitle
        title="The data center went routed. Layer 2 didn't follow."
        sub="Modern fabrics route everything as IP — but workloads still expect a flat L2 segment, and the freedom to live (and move) anywhere on it."
      />
      <div style={{ position: 'relative', height: 580, marginTop: 8 }}>
        ...
        <svg width={720} height={300} style={{ position: 'absolute', left: 0, top: 70, pointerEvents: 'none' }}>
          <line x1={210} y1={70} x2={258} y2={70} stroke={C.green} strokeWidth={2} strokeDasharray="5 5" style={{ opacity: gate(localTime, 1.4, 0.5) }} />
          <line x1={462} y1={70} x2={510} y2={70} stroke={C.steel} strokeWidth={2} strokeDasharray="5 5" style={{ opacity: gate(localTime, 2.2, 0.5) }} />
          <text x={360} y={250} textAnchor="middle" fontFamily={F.m} fontSize={15} fill={C.red} style={{ opacity: gate(localTime, 1.8, 0.5) }}>
            ✕ broadcast / ARP can't cross
          </text>
        </svg>
        ...
        <Rise at={1.9} dur={0.6} style={{ position: 'absolute', right: 0, top: 80, width: 560 }}>
          <div style={{ background: C.panel, border: `1.5px solid ${C.line}`, borderRadius: 14, padding: '34px 38px' }}>
            ...
            <div style={{ font: `600 76px ${F.d}`, color: C.amber, letterSpacing: '-0.02em', fontVariantNumeric: 'tabular-nums' }}>{vlanCount.toLocaleString()}</div>
            <div style={{ font: `400 18px/1.5 ${F.s}`, color: C.dim, marginTop: 8 }}>usable segments — a hard ceiling for multi-tenant clouds with thousands of tenants.</div>
          </div>
        </Rise>
...
// ── SCENE 02 — OVERLAY / UNDERLAY ────────────────────────────────────────────
function SceneOverlay() {
  const { localTime } = useSprite();
  return (
    <SceneFrame>
      <SceneTitle
        title="The fix: build a virtual network on top of the physical one."
        sub="VXLAN is an overlay. The physical IP network becomes a dumb, fast transport — the underlay — and virtual L2 networks are tunneled across it."
      />
      <div style={{ position: 'relative', height: 560, marginTop: 16 }}>
        ...
        <Rise at={1.2} dur={0.6} style={{ position: 'absolute', left: 0, top: 296, right: 0 }}>
          <div style={{ background: C.steelSoft, border: `1.5px solid ${C.steelLine}`, borderRadius: 16, padding: '26px 36px', height: 168, boxSizing: 'border-box' }}>
            ...
            <div style={{ display: 'flex', gap: 14, marginTop: 18 }}>
              <Chip color={C.steel}>OSPF / IS-IS / eBGP</Chip>
              <Chip color={C.steel}>ECMP</Chip>
              <Chip color={C.steel}>just IP routing</Chip>
            </div>
          </div>
        </Rise>
...
function SceneVtep() {
  ...
  return (
    <SceneFrame>
      <SceneTitle
        title="Who does the wrapping? The VTEP."
        sub="A VXLAN Tunnel Endpoint is where encapsulation and decapsulation happen — usually the leaf switch, sometimes a host's virtual switch. It's the door between overlay and underlay."
      />
      <div style={{ position: 'relative', height: 560, marginTop: 24 }}>
        ...
        <svg width={1656} height={560} style={{ position: 'absolute', left: 0, top: 0, pointerEvents: 'none' }}>
          ...
          <line x1={550} y1={215} x2={1106} y2={215} stroke={C.greenLine} strokeWidth={3} strokeDasharray="3 8" strokeLinecap="round" style={{ opacity: gate(localTime, 1.5, 0.6) }} />
          <rect x={690} y={188} width={276} height={54} rx={10} fill={C.bg} stroke={C.greenLine} strokeWidth={1.5} style={{ opacity: gate(localTime, 1.8, 0.5) }} />
          <text x={828} y={221} textAnchor="middle" fontFamily={F.m} fontSize={15} fill={C.green} style={{ opacity: gate(localTime, 1.9, 0.5) }}>
            VXLAN tunnel · UDP/IP
          </text>
        </svg>
    ...
    <div style={{ width: w, opacity: o, transform: `translateX(${tx}px)`, flexShrink: 0 }}>
      ...
      <div
        style={{
          height: 92,
          borderRadius: 9,
          background: dim ? C.panel : color + '22',
          border: `1.5px solid ${dim ? C.line : color}`,
          display: 'flex',
          flexDirection: 'column',
          alignItems: 'center',
          justifyContent: 'center',
...
function SceneEncap() {
  ...
  return (
    <SceneFrame>
      <SceneTitle
        title="Anatomy of a VXLAN packet"
        sub="The original frame is never touched. The VTEP just prepends four new headers so the underlay can route it from one VTEP IP to another."
      />
        ...
        <Rise at={3.9} dur={0.6} style={{ position: 'absolute', bottom: 0, left: 0, right: 0 }}>
          <div style={{ background: C.greenSoft, border: `1.5px solid ${C.greenLine}`, borderRadius: 14, padding: '22px 28px' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              ] as [string, string, string, boolean][]).map(([n, b, s, d], i) => (
                <div
                  key={i}
                  style={{
                    flex: n === 'VNI' ? 2.4 : n === 'Reserved' && b === '24 bits' ? 2.4 : 1,
                    background: d ? C.panel : n === 'VNI' ? C.green + '2e' : C.panel2,
                    border: `1.5px solid ${n === 'VNI' ? C.green : C.line}`,
                    borderRadius: 9,
                    padding: '14px 12px',
                    textAlign: 'center',
                  }}
...
function SceneVni() {
  ...
  return (
    <SceneFrame>
      <SceneTitle
        title="The VNI: 24 bits that replace the VLAN"
        sub="The VXLAN Network Identifier scopes each virtual segment. It rides inside every encapsulated packet, so the egress VTEP knows exactly which tenant L2 domain the frame belongs to."
      />
      <div style={{ position: 'relative', height: 540, marginTop: 16, display: 'flex', gap: 40 }}>
        <div style={{ flex: 1.1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <Rise at={0.7} dur={0.6}>
            ...
            <div style={{ font: `700 92px ${F.d}`, color: C.green, letterSpacing: '-0.03em', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{big.toLocaleString()}</div>
...
function SceneFabric() {
  ...
  const leafCx = (i: number) => leaves[i] + lw / 2;
  const spineCx = (i: number) => spines[i].x + sw / 2;
  return (
    <SceneFrame>
      <SceneTitle
        title="Where this lives: the spine-leaf fabric"
        sub="A Clos topology. Every leaf connects to every spine. Spines are pure IP transit; leaves carry the hosts and act as the VTEPs."
      />
      <div style={{ position: 'relative', height: 560, marginTop: 16 }}>
        <svg width={1656} height={520} style={{ position: 'absolute', left: 0, top: 0, pointerEvents: 'none' }}>
          {spines.map((_s, si) =>
            leaves.map((_lx, li) => {
              ...
              return (
                <line key={`${si}-${li}`} x1={spineCx(si)} y1={sy + sh} x2={leafCx(li)} y2={ly} stroke={C.steelLine} strokeWidth={1.5} style={{ opacity: gate(localTime, at, 0.4) * 0.7 }} />
              );
            }),
          )}
          <path
            d={`M ${leafCx(0)} ${ly} C ${leafCx(0)} 150, ${leafCx(3)} 150, ${leafCx(3)} ${ly}`}
            fill="none"
            stroke={C.green}
            strokeWidth={3}
            strokeDasharray="3 8"
...
function SceneFlood() {
  ...
  return (
    <SceneFrame>
      <SceneTitle
        title="First approach: flood-and-learn"
        sub="Before any control protocol, a VTEP discovers remote MACs the way a switch always has — by flooding the unknowns and learning from what comes back."
      />
      <div style={{ position: 'relative', height: 560, marginTop: 8 }}>
        ...
        <svg width={1656} height={250} style={{ position: 'absolute', left: 0, top: 0, pointerEvents: 'none' }}>
          {[1, 2, 3].map((i) => {
            ...
            return (
              <g key={i}>
                <line x1={leafCx(0)} y1={ly} x2={leafCx(0) + (leafCx(i) - leafCx(0)) * prog} y2={ly} stroke={C.amber} strokeWidth={2.5} strokeDasharray="4 6" strokeLinecap="round" />
              </g>
            );
          })}
        </svg>
        <Rise at={1.0} dur={0.5} style={{ position: 'absolute', left: 120, top: ly + lh + 26, width: 700 }}>
          ...
          <div style={{ font: `400 17px/1.5 ${F.s}`, color: C.dim, marginTop: 6 }}>
            flooded to <i>every</i> other VTEP in the VNI — via an underlay multicast group, or ingress (head-end) replication that unicasts a copy to each peer.
          </div>
        </Rise>
        <Rise at={2.6} dur={0.5} style={{ position: 'absolute', right: 0, top: ly + lh + 26, width: 480 }}>
          <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 12, padding: '18px 22px' }}>
            ...
            <div style={{ font: `400 15px/1.5 ${F.s}`, color: C.dim, marginTop: 6 }}>
              When the target answers, each VTEP records <span style={{ color: C.green, fontFamily: F.m }}>MAC ↔ remote-VTEP</span> from the decapsulated frame. Data-plane learning, exactly like a bridge.
            </div>
          </div>
        </Rise>
        <Rise at={3.4} dur={0.6} style={{ position: 'absolute', bottom: 0, left: 0, right: 0 }}>
...
function SceneEvpn() {
  ...
  return (
    <SceneFrame>
      <SceneTitle
        title="The modern way: a BGP EVPN control plane"
        sub="Separate control from data. MP-BGP with the EVPN address family distributes reachability, so every VTEP learns who lives where — before a single packet is flooded."
      />
      <div style={{ position: 'relative', height: 560, marginTop: 8 }}>
  ...
  return (
    <div
      style={{
        ...
        background: primary ? C.greenSoft : C.panel,
        border: `1.5px solid ${primary ? C.greenLine : C.line}`,
        borderRadius: 13,
        padding: '20px 22px',
        display: 'flex',
        flexDirection: 'column',
...
function SceneRouteTypes() {
  ...
  return (
    <SceneFrame>
      <SceneTitle
        title="The EVPN route types you'll actually meet"
        sub="EVPN is a single BGP address family carrying several Network Layer Reachability types. Three do the day-to-day work; two handle multihoming."
      />
      <div style={{ position: 'relative', height: 560, marginTop: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18, marginBottom: 18 }}>
          <RouteCard localTime={localTime} at={0.5} primary accent={C.green} n="Type 2" name="MAC / IP Advertisement" role="the workhorse" detail="Advertises a host's MAC, optionally its IP, plus the L2 VNI (and an L3 VNI for routing). This is how every VTEP learns where a host lives — and powers MAC mobility when a VM moves." />
          <RouteCard localTime={localTime} at={0.9} primary accent={C.green} n="Type 3" name="Inclusive Multicast (IMET)" role="builds the flood list" detail="Each VTEP announces 'I'm in VNI 10010.' Peers use these to build the per-VNI ingress-replication list — so the rare BUM that must flood reaches only the right VTEPs." />
          <RouteCard localTime={localTime} at={1.3} primary accent={C.green} n="Type 5" name="IP Prefix Route" role="routes between subnets" detail="Advertises IP prefixes (not host MACs) for inter-subnet, L3 routing across the fabric — including reaching external networks and silent hosts, via an L3 VNI / VRF." />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1.1fr', gap: 18 }}>
          <RouteCard localTime={localTime} at={2.0} accent={C.steel} n="Type 1" name="Ethernet Auto-Discovery" role="multihoming" detail="Per-ES / per-EVI. Enables fast 'mass withdrawal' on link failure and aliasing (load-balancing) to a dual-homed host." />
          <RouteCard localTime={localTime} at={2.3} accent={C.steel} n="Type 4" name="Ethernet Segment" role="multihoming" detail="Discovers which VTEPs share an Ethernet Segment (ESI), elects a Designated Forwarder, and enforces split-horizon." />
...
function SceneArp() {
  ...
  return (
    <SceneFrame>
      <SceneTitle
        title="A free win: ARP suppression"
        sub="Because Type-2 routes already carry every host's IP-to-MAC binding, the local leaf can answer ARP itself — the request never has to flood the fabric."
      />
      <div style={{ position: 'relative', height: 560, marginTop: 24 }}>
        ...
        <Rise at={3.4} dur={0.6} style={{ position: 'absolute', bottom: 0, left: 0, right: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, justifyContent: 'center', background: C.greenSoft, border: `1px solid ${C.greenLine}`, borderRadius: 12, padding: '18px 28px' }}>
            <div style={{ width: 9, height: 9, borderRadius: 9, background: C.green }} />
            <div style={{ font: `500 21px ${F.s}`, color: C.text }}>The leaf intercepts ARP and replies locally — broadcast suppressed, fabric stays calm even at huge scale.</div>
          </div>
        </Rise>
      </div>
    </SceneFrame>
...
function SceneWalk() {
  ...
  const encapsulated = tt > 0.18 && tt < 0.82;
  ...
  return (
    <SceneFrame>
      <SceneTitle
        title="Putting it together: one packet, end to end"
        sub="Host A to Host B — different leaves, same VNI 10010. Watch the encapsulation appear in the fabric and vanish on the far side."
      />
      <div style={{ position: 'relative', height: 540, marginTop: 8 }}>
        ...
        <svg width={1656} height={420} style={{ position: 'absolute', left: 0, top: 0, pointerEvents: 'none' }}>
          ...
          <polyline
            points={pts.slice(0, si + 1).concat([{ x: px, y: py }]).map((p) => `${p.x},${p.y}`).join(' ')}
            fill="none"
            stroke={encapsulated ? C.green : C.steel}
            strokeWidth={3}
            strokeLinecap="round"
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 14 }}>
          {steps.map(([h, s, thr], i) => {
            ...
            return (
              <div key={i} style={{ opacity: o, background: active ? C.greenSoft : C.panel, border: `1px solid ${active ? C.greenLine : C.line}`, borderRadius: 11, padding: '16px 16px', transition: 'background 200ms, border-color 200ms' }}>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, padding: '110px 132px', boxSizing: 'border-box' }}>
      <Rise at={0.2} dur={0.6}>
        ...
        <div style={{ font: `600 58px/1.05 ${F.d}`, color: C.text, letterSpacing: '-0.02em', maxWidth: 1200 }}>VXLAN = Layer-2 segments, routed at data-center scale.</div>
      </Rise>
...
function Chrome({ sections, total }: { sections: TimedScene[]; total: number }) {
  const t = useTime();
  let cur = sections[0];
  let idx = 0;
...
function Backdrop() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: C.bg, overflow: 'hidden' }}>
      <div style={{ position: 'absolute', inset: -46, backgroundImage: `radial-gradient(${C.line} 1px, transparent 1px)`, backgroundSize: '46px 46px', backgroundPosition: `${off}px ${off * 0.6}px`, opacity: 0.5 }} />
...
// ── timeline assembly ────────────────────────────────────────────────────────
const SCENES: { id: string; label: string; dur: number; comp: React.ComponentType }[] = [
...
let _acc = 0;
const TIMED: TimedScene[] = SCENES.map((s) => {
  const o = { ...s, start: _acc, end: _acc + s.dur };
  _acc += s.dur;
  return o;
});
const TOTAL = _acc;
